# Headroom — Linux desktop client

A native Qt Quick client for the existing usage API. Designed for KDE / Wayland,
...
```bash
cmake -S clients/linux -B clients/linux/build -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build clients/linux/build
clients/linux/build/headroom
```

Install the executable, application-menu entry, and icon for your user:

```bash
cmake --install clients/linux/build --prefix "$HOME/.local"
```
...
```bash
ctest --test-dir clients/linux/build --output-on-failure
QT_QPA_PLATFORM=offscreen QT_QUICK_BACKEND=software \
  clients/linux/build/headroom --demo --screenshot /tmp/headroom.png
```
