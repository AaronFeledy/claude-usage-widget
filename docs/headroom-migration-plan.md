...
Use a fresh review of the implementation rather than only the plan checklist. Run the migrated legacy behavior scenarios against the new packaged application, fix discovered issues in bounded follow-up work, and update the PR around the final implementation. Validate the Linux package on this desktop and Windows package on a native host or runner. ArrowOne's Windows side may provide interactive validation through WSL interop, but use isolated configuration and synthetic/local fixture servers so its real backend stays unchanged.
...
- Remote: reverse-proxy prefix, bearer auth rejection/recovery, provider auth failure, stale data/retry, endpoint switch, no unintended sidecar or credential forwarding over plain HTTP.
...
Batch 1 implementation is ready for coordinator review. Local Qt 6.11.1 Linux validation passed fresh KDE-enabled and KDE-disabled builds, all six test executables in both builds, an offscreen `--demo --screenshot` load, the `clients/linux` compatibility build, and a staged source install containing `bin/headroom`. The workflow pins Qt 6.8.3 with MSVC 2022 on `windows-2022` and records the `win64_msvc2022_arm64` kit for the later packaging batch. The first native CI attempt stopped in Qt installation because `qtsvg` was incorrectly requested as an optional aqt module; Qt's repository publishes it inside the base architecture package, so the workflow now requests only the separate `qtshadertools` module while CMake continues to require `Qt6::Svg`. The next run compiled Linux and reached the Windows link: Linux exposed request-header case differences between Qt 6.8 and 6.11, and MSVC reported duplicate manifest resource ID 1. The fixture assertions now compare HTTP header names case-insensitively while preserving exact credential values, and MSVC merges the Headroom manifest through `/MANIFESTINPUT` while the RC supplies icon/version resources. The following run passed the pinned-Qt Linux build, all six tests, and its screenshot; Windows built and passed five tests, but the UI test failed without QtTest output because it was linked as a GUI executable. Test targets now explicitly use the console subsystem and text output, CI prints `LastTest.log` on failure, and the demo screenshot runs after a test failure when the application built. Native Windows UI-test and screenshot evidence remains pending until that diagnostic rerun; this record does not claim they passed.
