...
Batch 1 implementation is ready for coordinator review. Local Qt 6.11.1 Linux validation passed fresh KDE-enabled and KDE-disabled builds, all six test executables in both builds, an offscreen `--demo --screenshot` load, the `clients/linux` compatibility build, and a staged source install containing `bin/headroom`. The workflow pins Qt 6.8.3 with MSVC 2022 on `windows-2022` and records the `win64_msvc2022_arm64` kit for the later packaging batch. The first native CI attempt stopped in Qt installation because `qtsvg` was incorrectly requested as an optional aqt module; Qt's repository publishes it inside the base architecture package, so the workflow now requests only the separate `qtshadertools` module while CMake continues to require `Qt6::Svg`. Native Windows x64 and pinned-Qt Linux build/test/screenshot evidence remains pending until the corrected CI rerun passes; this record does not claim those runs have passed.
