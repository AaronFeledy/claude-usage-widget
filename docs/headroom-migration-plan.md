...
The current native Windows x64/ARM64 and Linux results, CI artifacts, and remaining physical-desktop limitations are recorded in [PR #10](https://github.com/AaronFeledy/claude-usage-widget/pull/10). Testing through the Windows ChatGPT app could not be dispatched because ArrowOne was not connected to this session; native CI rendering and fixture execution are not presented as that physical-host test.
