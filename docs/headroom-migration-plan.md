...
Batch 1 implementation is ready for coordinator review. Local Qt 6.11.1 Linux validation passed fresh KDE-enabled and KDE-disabled builds, all six test executables in both builds, an offscreen `--demo --screenshot` load, the `clients/linux` compatibility build, and a staged source install containing `bin/headroom`. The workflow pins Qt 6.8.3 with MSVC 2022 on `windows-2022` and records the `win64_msvc2022_arm64` kit for the later packaging batch. Native Windows x64 build/test/screenshot evidence remains pending until the coordinator pushes and checks PR CI; this record does not claim that run has passed.
