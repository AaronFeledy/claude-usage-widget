...
func (f *updateFixture) serve(writer http.ResponseWriter, request *http.Request) {
	...
	if strings.HasSuffix(request.URL.Path, "/latest") || strings.Contains(request.URL.Path, "/tags/") {
		...
		assets := []githubAsset{
			{Name: "Headroom-v" + f.version + manifestSuffix, URL: f.server.URL + "/download/release", Size: int64(len(manifestBytes)) + f.manifestSizeDelta},
...
func (f *updateFixture) release() (ReleaseManifest, []byte) {
	...
	for _, target := range [][2]string{{"linux", "x86_64"}, {"windows", "arm64"}, {"windows", "x86_64"}, {"macos", "x86_64"}, {"macos", "arm64"}} {
